...
function readCachedSignedAssetUrl(storage, assetKey) {
  try {
    const raw = storage.getItem(`assetSignedUrl:${assetKey}`);
    if (!raw) return null;
    ...
    signedAssetUrlCache.set(assetKey, {
      url: parsed.url,
      ts: Number(parsed.ts),
...
function getCachedSignedAssetUrl(assetKey) {
  if (!assetKey) return null;

  const memoryCached = signedAssetUrlCache.get(assetKey);
  if (memoryCached?.url && Date.now() < Number(memoryCached.expiresAt || 0)) {
    return memoryCached.url;
  }

  const sessionCached = canUseSessionStorage() ? readCachedSignedAssetUrl(sessionStorage, assetKey) : null;
  if (sessionCached) return sessionCached;

  return canUseLocalStorage() ? readCachedSignedAssetUrl(localStorage, assetKey) : null;
}

function setCachedSignedAssetUrl(assetKey, url, expiresInSec = 60 * 30) {
  ...
  signedAssetUrlCache.set(assetKey, payload);

  try {
    const raw = JSON.stringify(payload);
    if (canUseSessionStorage()) sessionStorage.setItem(`assetSignedUrl:${assetKey}`, raw);
    if (canUseLocalStorage()) localStorage.setItem(`assetSignedUrl:${assetKey}`, raw);
  } catch {
...
export async function getSignedAssetUrl(assetKey, { expiresInSec = 60 * 30 } = {}) {
  ...
  const cached = getCachedSignedAssetUrl(assetKey);
  ...
  if (USE_SERVER_SIGNING && token) {
    try {
      ...
      if (signed?.signedUrl) {
        setCachedSignedAssetUrl(assetKey, signed.signedUrl, expiresInSec);
  ...
  if (latestPath) {
    ...
    if (!error && data?.signedUrl) {
      setCachedSignedAssetUrl(assetKey, data.signedUrl, expiresInSec);
