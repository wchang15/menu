...
const BUCKET = 'assets';
const LEGACY_SIGNED_ASSET_URL_MAX_AGE_MS = 1000 * 60 * 25;
...
function signedAssetCacheKey(assetKey, userId) {
  return `${userId || 'anon'}:${assetKey}`;
}

function signedAssetStorageKey(assetKey, userId) {
  return `assetSignedUrl:${signedAssetCacheKey(assetKey, userId)}`;
}

function readCachedSignedAssetUrl(storage, assetKey, userId) {
  try {
    const cacheKey = signedAssetCacheKey(assetKey, userId);
    const raw = storage.getItem(signedAssetStorageKey(assetKey, userId));
    if (!raw) return null;
    const parsed = JSON.parse(raw);
    if (!parsed?.url || !parsed?.ts) return null;
    const expiresAt = Number(parsed.expiresAt || 0);
    const legacyExpiresAt = Number(parsed.ts) + LEGACY_SIGNED_ASSET_URL_MAX_AGE_MS;
    if (Date.now() > (expiresAt || legacyExpiresAt)) return null;

    signedAssetUrlCache.set(cacheKey, {
      url: parsed.url,
      ts: Number(parsed.ts),
      expiresAt: expiresAt || legacyExpiresAt,
    });
    return parsed.url;
  } catch {
    return null;
...
function getCachedSignedAssetUrl(assetKey, userId) {
  if (!assetKey) return null;
  const cacheKey = signedAssetCacheKey(assetKey, userId);

  const memoryCached = signedAssetUrlCache.get(cacheKey);
  if (memoryCached?.url && Date.now() < Number(memoryCached.expiresAt || 0)) {
    return memoryCached.url;
  }

  const sessionCached = canUseSessionStorage() ? readCachedSignedAssetUrl(sessionStorage, assetKey, userId) : null;
  if (sessionCached) return sessionCached;

  return canUseLocalStorage() ? readCachedSignedAssetUrl(localStorage, assetKey, userId) : null;
}

function setCachedSignedAssetUrl(assetKey, url, expiresInSec = 60 * 30, userId = null) {
  if (!assetKey || !url) return;

  const cacheKey = signedAssetCacheKey(assetKey, userId);
  const now = Date.now();
  const safeExpiresInMs = Math.max(1000 * 60, Number(expiresInSec || 60 * 30) * 1000 - 1000 * 60);
  const payload = { url, ts: now, expiresAt: now + safeExpiresInMs };
  signedAssetUrlCache.set(cacheKey, payload);

  try {
    const raw = JSON.stringify(payload);
    if (canUseSessionStorage()) sessionStorage.setItem(signedAssetStorageKey(assetKey, userId), raw);
    if (canUseLocalStorage()) localStorage.setItem(signedAssetStorageKey(assetKey, userId), raw);
  } catch {
    // ignore cache write failures
  }
}

function clearCachedSignedAssetUrl(assetKey, userId = null) {
  if (!assetKey) return;
  const cacheKey = signedAssetCacheKey(assetKey, userId);
  signedAssetUrlCache.delete(cacheKey);
  try {
    if (canUseSessionStorage()) sessionStorage.removeItem(signedAssetStorageKey(assetKey, userId));
    if (canUseLocalStorage()) localStorage.removeItem(signedAssetStorageKey(assetKey, userId));
  } catch {
...
function buildSafeStorageFilename(filename, fallbackBase = 'file') {
  ...
  const extPart = (match?.[2] || '').trim();

  const safeBase = sanitizeStorageSegment(basePart, fallbackBase);
  const safeExt = extPart ? `.${sanitizeStorageSegment(extPart.slice(1), 'bin')}` : '';
...
function latestMirrorPath(userId, assetKey) {
  return `${userId}/__latest/${assetKey}`;
}

function latestMetaPath(userId, assetKey) {
  return `${userId}/__latest_meta/${assetKey}.json`;
}

async function writeLatestPointers({ userId, assetKey, fileOrBlob, contentType, versionPath }) {
  if (!userId || !assetKey || !fileOrBlob || !versionPath) return;
  ...
  const mirrorPath = latestMirrorPath(userId, assetKey);
  await supabase.storage.from(BUCKET).upload(mirrorPath, fileOrBlob, {
    contentType: inferredContentType,
    upsert: true,
    cacheControl: '0',
  });

  const metaBlob = new Blob([JSON.stringify({ version: versionPath, updatedAt: Date.now() })], {
    type: 'application/json',
  });
  await supabase.storage.from(BUCKET).upload(latestMetaPath(userId, assetKey), metaBlob, {
...
async function readLatestMeta({ userId, assetKey }) {
  if (!userId || !assetKey) return null;
  try {
    const { data, error } = await supabase.storage.from(BUCKET).download(latestMetaPath(userId, assetKey));
...
export async function getSignedAssetUrl(assetKey, { expiresInSec = 60 * 30 } = {}) {
  ...
  const cached = getCachedSignedAssetUrl(assetKey, userId);
  ...
  if (USE_SERVER_SIGNING && token) {
    try {
      ...
      if (signed?.signedUrl) {
        setCachedSignedAssetUrl(assetKey, signed.signedUrl, expiresInSec, userId);
  ...
  // ✅ 2-1) latest meta/mirror 우선
  const latestMeta = await readLatestMeta({ userId, assetKey });
  const latestPath = latestMeta?.version || (await listLatestVersionPath({ userId, assetKey }));
  if (latestPath) {
    const { data, error } = await supabase.storage.from(BUCKET).createSignedUrl(latestPath, expiresInSec);
    if (!error && data?.signedUrl) {
      setCachedSignedAssetUrl(assetKey, data.signedUrl, expiresInSec, userId);
...
export async function uploadAsset({ assetKey, file, contentType }) {
  ...
  const safeFilename = buildSafeStorageFilename(inferredFilename, sanitizeStorageSegment(assetKey, 'asset'));
  ...
  clearCachedSignedAssetUrl(assetKey, userId);
  const objectPath = `${userId}/${assetKey}/${Date.now()}-${safeFilename}`;
  ...
  await writeLatestPointers({ userId, assetKey, fileOrBlob: file, contentType: inferredContentType, versionPath: objectPath });
...
export async function getLatestAssetVersion(assetKey) {
  ...
  const latestMeta = await readLatestMeta({ userId, assetKey });
  if (latestMeta?.version) return { version: latestMeta.version, path: latestMeta.version, updatedAt: latestMeta.updatedAt || null };
