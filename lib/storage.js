...
import { get, set, del, clear, keys as idbKeys } from 'idb-keyval';
...
export async function syncBlobFromCloud(key, { onRemoteDiff } = {}) {
  try {
    ...
    if (onRemoteDiff && (!localVersion || !remotePath || localVersion !== remotePath)) onRemoteDiff();

    const preferredPath = looksLikeFilePath(remotePath) ? remotePath : looksLikeFilePath(localVersion) ? localVersion : null;
    const blob = preferredPath ? await downloadAssetBlobByPath(preferredPath) : await downloadAssetBlob(key);
...
export async function syncJsonFromCloud(key, { onRemoteDiff, force = false } = {}) {
  try {
    const { data: sessionData } = await supabase.auth.getSession();
    if (!sessionData?.session) return { updated: false };
    ...
    if (!force && remotePath && localVersion && localVersion === remotePath) return { updated: false };
    ...
    const preferredPath = looksLikeFilePath(remotePath) ? remotePath : looksLikeFilePath(localVersion) ? localVersion : null;
    if (preferredPath) {
      const blob = await downloadAssetBlobByPath(preferredPath);
      if (blob) {
        try {
...
export async function clearUserLocalCache(userId) {
  ...
  try {
    ...
    await Promise.all(
      allKeys
        .filter((key) => typeof key === 'string' && key.startsWith(`${safeUserId}__`))
        .map((key) => del(key))
    );
  } catch (error) {
    console.error('clearUserLocalCache failed', error);
  }
}
